    ...
    <div class="container">
      <nav class="navbar navbar-expand-lg">
          ...
          <ul id="mainMenu" class="navbar-nav mobile-item mx-auto">
            ...
            @foreach ($links as $link)
              ...
              @else
                ...
                <li class="nav-item">
                  <a href="#" class="nav-link toggle">{{ $link['text'] }} <i class="fal fa-plus"></i></a>
        ...
        <div class="more-option mobile-item">
          ...
          @auth
            <div class="item">
              <a href="{{ route('user.client.dashboard', getParam()) }}" class="btn-icon" target="_self"
                aria-label="User" title="{{ __('Dashboard') }}">
                <i class="fal fa-user-circle"></i>
              </a>
            </div>
          @else
            <div class="item">
              <a href="{{ route('user.client.login', getParam()) }}" class="btn-icon" target="_self" aria-label="User"
                title="{{ __('Login') }}">
          ...
          @if ($userBs->is_quote)
            <div class="item">
              <a href="{{ route('user.front.reservation', getParam()) }}" class="btn btn-md btn-primary radius-0"
